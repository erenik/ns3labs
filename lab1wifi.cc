...
std::string PlmStr(int which)
{
  std::string str = "Unknown/None";
  switch(which)
  {
    case 0: return "None";
    case TWO_RAY: return "Two-Ray ground";
    case COST_231: return "Cost231";
    case FRIIS: return "Friis";
  }
  return str;
}
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue("plm", "propagation loss model", plm);
  cmd.Parse (argc,argv);

  std::cout<<"\nPropagation loss model selected: "<<PlmStr(plm)<<" ("<<plm<<")"<<std::endl;
  if (nWifi > 18)
    {
...
/////////////////////////////Nodes/////////////////////////////  
 ...
 /////////////////////////////Wi-Fi part///////////////////////////// 
  ...
  /// Install propagation loss model.
  if (plm == TWO_RAY)
    lossModel = CreateObject<TwoRayGroundPropagationLossModel> (); //create a pointer for propagation loss model
  else if (plm == COST_231)
    lossModel = CreateObject<Cost231PropagationLossModel> (); //create a pointer for propagation loss model
  else if (plm == FRIIS)
    lossModel = CreateObject<FriisPropagationLossModel> (); //create a pointer for propagation loss model
  if (lossModel)
